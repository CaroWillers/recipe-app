...
// node_modules/@angular/fire/node_modules/@firebase/functions/dist/esm-node/index.node.esm.js
...
var FunctionsError2 = class extends FirebaseError {
  constructor(code, message, details) {
...
var FunctionsService2 = class {
   ...
   * @param app - The FirebaseApp to use.
   */
  constructor(app, authProvider, messagingProvider, appCheckProvider, regionOrCustomDomain = DEFAULT_REGION2, fetchImpl) {
    this.app = app;
    this.fetchImpl = fetchImpl;
    this.emulatorOrigin = null;
    this.contextProvider = new ContextProvider2(authProvider, messagingProvider, appCheckProvider);
    this.cancelAllRequests = new Promise((resolve) => {
      this.deleteService = () => {
        return Promise.resolve(resolve());
      };
...
function postJSON2(url, body, headers, fetchImpl) {
  return __async(this, null, function* () {
    headers["Content-Type"] = "application/json";
    let response;
    try {
...
function callAtURL2(functionsInstance, url, data, options) {
  return __async(this, null, function* () {
    data = encode2(data);
    const body = {
      data
    ...
    const response = yield Promise.race([postJSON2(url, body, headers, functionsInstance.fetchImpl), failAfterHandle.promise, functionsInstance.cancelAllRequests]);
    failAfterHandle.cancel();
    if (!response) {
...
function registerFunctions2(fetchImpl, variant) {
  const factory = (container, {
    instanceIdentifier: regionOrCustomDomain
  }) => {
    const app = container.getProvider("app").getImmediate();
    ...
    return new FunctionsService2(app, authProvider, messagingProvider, appCheckProvider, regionOrCustomDomain, fetchImpl);
  };
...
registerFunctions2(import_undici.fetch, "node");
