import {
  isPlatformBrowser
} from "./chunk-KYUD4HKN.js";
import "./chunk-ZGSPMIHC.js";
import {
  Component,
  ErrorFactory,
  FirebaseApp,
...
var traceUntil = function(name2, test, options) {
  return function(source$) {
    return new Observable(function(subscriber) {
      var traceSubscription = trace$(name2).subscribe();
...
var traceWhile = function(name2, test, options) {
  return function(source$) {
    return new Observable(function(subscriber) {
      var traceSubscription;
      return source$.pipe(tap(function(a) {
        if (test(a)) {
          traceSubscription = traceSubscription || trace$(name2).subscribe();
        } else {
...
var traceUntilComplete = function(name2) {
  return function(source$) {
    return new Observable(function(subscriber) {
      var traceSubscription = trace$(name2).subscribe();
...
var traceUntilFirst = function(name2) {
  return function(source$) {
    return new Observable(function(subscriber) {
      var traceSubscription = trace$(name2).subscribe();
      return source$.pipe(tap(function() {
...
// node_modules/@angular/fire/node_modules/@firebase/performance/dist/esm/index.esm2017.js
...
function logTrace(trace3) {
  const settingsService = SettingsService.getInstance();
  if (!settingsService.instrumentationEnabled && trace3.isAuto) {
    return;
  }
  if (!settingsService.dataCollectionEnabled && !trace3.isAuto) {
    return;
  }
  if (!Api.getInstance().requiredApisAvailable()) {
    return;
  }
  if (trace3.isAuto && getVisibilityState() !== VisibilityState.VISIBLE) {
    return;
  }
  if (isPerfInitialized()) {
    sendTraceLog(trace3);
  } else {
    getInitializationPromise(trace3.performanceController).then(() => sendTraceLog(trace3), () => sendTraceLog(trace3));
  }
}
function sendTraceLog(trace3) {
  if (!getIid()) {
    return;
  }
  const settingsService = SettingsService.getInstance();
...
function serializeTrace(trace3) {
  const traceMetric = {
    name: trace3.name,
    is_auto: trace3.isAuto,
    client_start_time_us: trace3.startTimeUs,
    duration_us: trace3.durationUs
  };
  if (Object.keys(trace3.counters).length !== 0) {
    traceMetric.counters = trace3.counters;
  }
  const customAttributes = trace3.getAttributes();
  if (Object.keys(customAttributes).length !== 0) {
    traceMetric.custom_attributes = customAttributes;
  }
  const perfMetric = {
    application_info: getApplicationInfo(trace3.performanceController.app),
...
var Trace = class _Trace {
  ...
  static createOobTrace(performanceController, navigationTimings, paintTimings, firstInputDelay) {
    ...
    const trace3 = new _Trace(performanceController, OOB_TRACE_PAGE_LOAD_PREFIX + route, true);
    const timeOriginUs = Math.floor(Api.getInstance().getTimeOrigin() * 1e3);
    trace3.setStartTime(timeOriginUs);
    if (navigationTimings && navigationTimings[0]) {
      trace3.setDuration(Math.floor(navigationTimings[0].duration * 1e3));
      trace3.putMetric("domInteractive", Math.floor(navigationTimings[0].domInteractive * 1e3));
      trace3.putMetric("domContentLoadedEventEnd", Math.floor(navigationTimings[0].domContentLoadedEventEnd * 1e3));
      trace3.putMetric("loadEventEnd", Math.floor(navigationTimings[0].loadEventEnd * 1e3));
    }
    ...
    if (paintTimings) {
      ...
      if (firstPaint && firstPaint.startTime) {
        trace3.putMetric(FIRST_PAINT_COUNTER_NAME, Math.floor(firstPaint.startTime * 1e3));
      }
      const firstContentfulPaint = paintTimings.find((paintObject) => paintObject.name === FIRST_CONTENTFUL_PAINT);
      if (firstContentfulPaint && firstContentfulPaint.startTime) {
        trace3.putMetric(FIRST_CONTENTFUL_PAINT_COUNTER_NAME, Math.floor(firstContentfulPaint.startTime * 1e3));
      }
      if (firstInputDelay) {
        trace3.putMetric(FIRST_INPUT_DELAY_COUNTER_NAME, Math.floor(firstInputDelay * 1e3));
      }
    }
    logTrace(trace3);
  }
  static createUserTimingTrace(performanceController, measureName) {
    const trace3 = new _Trace(performanceController, measureName, false, measureName);
    logTrace(trace3);
  }
};
